import type { Administration } from "./types";

export const administrationData: Administration = {
  chancellor: {
    hindi: {
      name: "महंत योगी आदित्यनाथ जी महाराज",
      designation: "कुलाधिपति / संस्थापक",
      university: "महायोगी गोरखनाथ विश्वविद्यालय गोरखपुर",
    },
    english: {
      name: "Mahant Yogi Adityanath Ji Maharaj",
      designation: "Chancellor / Founder",
      university: "Mahayogi Gorakhnath University Gorakhpur",
    },
    image: "/admin/YogiAdityanath.webp",
    email: "yogiadityanath72@gmail.com",
    website: "https://www.yogiadityanath.in",
    socials: {
      instagram: "https://instagram.com/yogiadityanath",
      facebook: "https://facebook.com/yogiadityanath",
      twitter: "https://twitter.com/myogiadityanath",
      youtube: "https://youtube.com/@yogiadityanath",
    },
  },
  proChancellor: {
    hindi: {
      name: "प्रो. उदय प्रताप सिंह",
      designation: "प्रो कुलाधिपति",
      university: "महायोगी गोरखनाथ विश्वविद्यालय गोरखपुर",
    },
    english: {
      name: "Prof. Uday Pratap Singh",
      designation: "Pro Chancellor",
      university: "Mahayogi Gorakhnath University Gorakhpur",
    },
    image: "/admin/drupsingh.jpg",
    email: "upsingh300@gmail.com",
    profilePdf: "https://mgug.ac.in/PDF/ProfUPSingh.pdf",
  },
  viceChancellor: {
    hindi: {
      name: "डॉ. सुरिंदर सिंह",
      designation: "कुलपति",
      university: "महायोगी गोरखनाथ विश्वविद्यालय गोरखपुर",
    },
    english: {
      name: "Dr. Surinder Singh",
      designation: "Vice Chancellor",
      university: "Mahayogi Gorakhnath University Gorakhpur",
    },
    image: "/admin/vcmgug.webp",
    email: "vc@mgug.ac.in",
    profilePdf: "https://mgug.ac.in/PDF/Dr_Surinder_Singh_Profile.pdf",
  },
  registrar: {
    hindi: {
      name: "डॉ. प्रदीप कुमार राव",
      designation: "कुलसचिव सह वित्त अधिकारी",
      university: "महायोगी गोरखनाथ विश्वविद्यालय गोरखपुर",
    },
    english: {
      name: "Dr. Pradeep Kumar Rao",
      designation: "Registrar Cum Finance Officer",
      university: "Mahayogi Gorakhnath University Gorakhpur",
    },
    image: "/admin/DRPradeepRao.jpg",
    email: "registrar@mgug.ac.in",
    profilePdf: "https://mgug.ac.in/PDF/DrPradeepRao.pdf",
  },
  financeOfficer: {
    hindi: {
      name: "डॉ. प्रदीप कुमार राव",
      designation: "कुलसचिव सह वित्त अधिकारी",
      university: "महायोगी गोरखनाथ विश्वविद्यालय गोरखपुर",
    },
    english: {
      name: "Dr. Pradeep Kumar Rao",
      designation: "Registrar Cum Finance Officer",
      university: "Mahayogi Gorakhnath University Gorakhpur",
    },
    image: "/admin/DRPradeepRao.jpg",
    email: "registrar@mgug.ac.in",
    profilePdf: "https://mgug.ac.in/PDF/DrPradeepRao.pdf",
  },
  controllerOfExamination: {
    hindi: {
      name: "श्री अमित कुमार सिंह",
      designation: "परीक्षा नियंत्रक",
      university: "महायोगी गोरखनाथ विश्वविद्यालय गोरखपुर",
    },
    english: {
      name: "Mr. Amit Kumar Singh",
      designation: "Controller Of Examination",
      university: "Mahayogi Gorakhnath University Gorakhpur",
    },
    image: "/admin/coe_amit_singh.webp",
    email: "coe@mgug.ac.in",
    profilePdf: "https://mgug.ac.in/administration/coe.php#",
  },
  chiefProctor: {
    hindi: {
      name: "डॉ. विमल कुमार दुबे",
      designation: "मुख्य प्रॉक्टर",
      university: "महायोगी गोरखनाथ विश्वविद्यालय गोरखपुर",
    },
    english: {
      name: "Dr. Vimal Kumar Dubey",
      designation: "Chief Proctor",
      university: "Mahayogi Gorakhnath University Gorakhpur",
    },
    image: "/admin/Dr_Shashi_Kant_Singh.jpg",
    email: "chief.proctor@mgug.ac.in",
  },
  deanAcademics: {
    hindi: {
      name: "डॉ. प्रशांत एस.",
      designation: "डीन एकेडमिक्स",
      university: "महायोगी गोरखनाथ विश्वविद्यालय गोरखपुर",
    },
    english: {
      name: "Dr. Prashanth S.",
      designation: "Dean Academics",
      university: "Mahayogi Gorakhnath University Gorakhpur",
    },
    image: "/admin/it-team/prashanthMurthy.jpg",
    email: "dean.academics@mgug.ac.in",
    profilePdf: "https://mgug.ac.in/PDF/PrashanthS.pdf",
  },
  deanIQAC: {
    hindi: {
      name: "डॉ. रघु राम अचार",
      designation: "डीन आईक्यूएसी",
      university: "महायोगी गोरखनाथ विश्वविद्यालय गोरखपुर",
    },
    english: {
      name: "Dr. Raghu Ram Achar",
      designation: "Dean IQAC",
      university: "Mahayogi Gorakhnath University Gorakhpur",
    },
    image: "/admin/raghuramAchar.jpg",
    email: "deaniqac@mgug.ac.in",
    profilePdf: "https://mgug.ac.in/PDF/RaghuRamAchar.pdf",
  },
  deanIFTSP: {
    hindi: {
      name: "डॉ. हरि ओम शरण",
      designation: "डीन आईएफटीएसपी",
      university: "महायोगी गोरखनाथ विश्वविद्यालय गोरखपुर",
    },
    english: {
      name: "Dr. Hari Om Sharan",
      designation: "Dean IFTSP",
      university: "Mahayogi Gorakhnath University Gorakhpur",
    },
    image: "/admin/dr_hari_om_sharan.jpg",
    email: "dean.planning@mgug.ac.in",
    profilePdf: "https://mgug.ac.in/PDF/HariOmSharan.pdf",
  },
  chiefVigilanceOfficer: {
    hindi: {
      name: "डॉ. शशि कांत सिंह",
      designation: "मुख्य सतर्कता अधिकारी",
      university: "महायोगी गोरखनाथ विश्वविद्यालय गोरखपुर",
    },
    english: {
      name: "Dr. Shashi Kant Singh",
      designation: "Chief Vigilance Officer",
      university: "Mahayogi Gorakhnath University Gorakhpur",
    },
    image: "/admin/Dr_Shashi_Kant_Singh.jpg",
    email: "cvo@mgug.ac.in",
  },
  ombudsPerson: {
    hindi: {
      name: "प्रो. वी. के. सिंह",
      designation: "लोकपाल",
      university: "पूर्व कुलपति, डीडीयू गोरखपुर विश्वविद्यालय",
    },
    english: {
      name: "Prof. V. K. Singh",
      designation: "Ombuds Person",
      university: "Former Vice-Chancellor, DDU Gorakhpur University",
    },
    image: "/admin/Prof_V_K_Singh.jpg",
    email: "singhvijaikrishna9@gmail.com",
  },
};
