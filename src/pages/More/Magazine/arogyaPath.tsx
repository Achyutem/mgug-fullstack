...
export default function ArogyaPathPage() {
  const { language } = UseLanguage();
  const [magazines, setMagazines] = useState<ArogyaPathItem[]>([]);
  const [currentPage, setCurrentPage] = useState(1);

  useEffect(() => {
    fetch(`${API_BASE_URL}/notificationsApi.php?type=13`)
      .then((res) => res.json())
      .then((data) => {
