...
import { arogyaPrabhaTitles } from "@/utils/moreData";
import type { ArogyaPrabhaItem, MagazineTitles } from "@/utils/types";

const ITEMS_PER_PAGE = 10;
const API_BASE_URL = "https://mgug.ac.in/api";

const renderMagazineTable = (
  items: ArogyaPrabhaItem[],
...
export default function ArogyaPrabhaPage() {
  const { language } = UseLanguage();
  const [magazines, setMagazines] = useState<ArogyaPrabhaItem[]>([]);
  const [currentPage, setCurrentPage] = useState(1);

  useEffect(() => {
    fetch(`${API_BASE_URL}/notificationsApi.php?type=15`) //
      .then((res) => res.json())
      .then((data) => {
        if (Array.isArray(data)) {
          setMagazines(data);
        } else {
          console.error("API did not return an array for Arogya Prabha:", data);
          setMagazines([]);
        }
      })
      .catch(console.error);
    ...
    <MainLayout>
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h1 className="text-4xl font-bold mb-8 text-orange-500 text-center">
          {arogyaPrabhaTitles.heading[language]}
        </h1>
        {renderMagazineTable(
          displayedMagazines,
          arogyaPrabhaTitles.headers,
          language
        )}
        <Pagination
