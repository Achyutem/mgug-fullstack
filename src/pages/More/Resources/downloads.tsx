...
const renderDownloadsTable = (items: DownloadItem[]) => (
      ...
      <table className="w-full text-left text-slate-800">
        <thead>
          <tr className="bg-orange-500/20 text-orange-600">
            <th className="p-4 font-semibold rounded-tl-xl">File Name</th>
...
export default function DownloadsPage() {
  ...
  useEffect(() => {
    fetch(`${API_BASE_URL}/notificationsApi.php?type=9`)
      .then((res) => res.json())
      .then(setDownloads)
      .catch(console.error);
  }, []);

  const displayedDownloads = downloads.slice(
  ...
  return (
    <MainLayout>
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        {downloads.length > 0 ? (
          <>
            {renderDownloadsTable(displayedDownloads)}
            <Pagination
